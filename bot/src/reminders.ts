...
import { getCurrentPeriod, getPaymentDeadline, hhmm, isSameLocalDate } from "./lib/dates.js";
...
export const startReminderCron = (bot: Bot) => {
  cron.schedule("* * * * *", async () => {
    ...
    for (const musician of musicians) {
      ...
      const type =
        isSameLocalDate(now, beforeDate) ? "before" : isSameLocalDate(now, dueAt) ? "due" : now > graceEndsAt ? "overdue" : null;
